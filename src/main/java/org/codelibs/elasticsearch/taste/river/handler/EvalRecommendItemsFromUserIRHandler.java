...
import org.codelibs.elasticsearch.taste.eval.GenericRecommenderIRStatsEvaluator;
import org.codelibs.elasticsearch.taste.eval.IRRecommenderEvaluatorFactory;
import org.codelibs.elasticsearch.taste.eval.IRStatistics;
import org.codelibs.elasticsearch.taste.eval.IRStatsEvaluatorFactory;
import org.codelibs.elasticsearch.taste.eval.RMSRecommenderEvaluator;
...
import org.codelibs.elasticsearch.taste.eval.RecommenderIRStatsEvaluator;
...
public class EvalRecommendItemsFromUserIRHandler extends RecommendationHandler {
	private RecommenderIRStatsEvaluator evaluator;

	public EvalRecommendItemsFromUserIRHandler(final RiverSettings settings,
	...
	protected void evaluate(final DataModel dataModel,
			...
			final RecommenderIRStatsEvaluator evaluator, final ObjectWriter writer,
			final EvaluationConfig config) {

		RandomUtils.useTestSeed();
		try {
			long time = System.currentTimeMillis();
			final IRStatistics evaluation = evaluator.evaluate(
					recommenderBuilder, null, dataModel, null, 5, GenericRecommenderIRStatsEvaluator.CHOOSE_THRESHOLD, 1.0);
			...
			targetObj.put("dataUsed", "100%");
			evaluationObj.put("target", targetObj);

			evaluationObj.put("precision", evaluation.getPrecision());
			evaluationObj.put("recall", evaluation.getRecall());
			evaluationObj.put("f1", evaluation.getF1Measure());
	...
	protected RecommenderIRStatsEvaluator createEvaluator(
		...
		try {
			...
			final IRStatsEvaluatorFactory recommenderEvaluatorFactory = (IRStatsEvaluatorFactory) clazz
					.newInstance();
			recommenderEvaluatorFactory.init(evaluatorSettings);
			final RecommenderIRStatsEvaluator evaluator = recommenderEvaluatorFactory.create();
